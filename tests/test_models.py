...
class TestLeituraDeInstante(unittest.TestCase):
    def test_iso_with_a_zulu_suffix(self):
        self.assertEqual(parse_instante("2026-09-12T15:20:22.362Z").year, 2026)

    def test_a_numeric_epoch_written_as_text(self):
        """A forma que originou toda esta família de projetos."""
        instante = parse_instante("1789226422362")
    ...
    def test_seconds_and_milliseconds_land_on_the_same_moment(self):
        self.assertEqual(parse_instante(1789226422), parse_instante(1789226422000))

    def test_a_naive_timestamp_is_assumed_utc(self):
        self.assertEqual(parse_instante("2026-09-12T15:20:22").tzinfo, timezone.utc)

    def test_nonsense_is_none_and_never_raises(self):
        for valor in (None, "", "amanhã", [], {}):
            self.assertIsNone(parse_instante(valor))


class TestSaudeDaChave(unittest.TestCase):
    def test_a_key_well_inside_its_validity_is_active(self):
        chave = VirtualKey({"key_alias": "k", "expires": daqui(days=30)})
        self.assertEqual(chave.health_status(), SAUDE_ATIVA)

    def test_a_key_inside_the_margin_is_expiring(self):
        chave = VirtualKey({"key_alias": "k", "expires": daqui(minutes=5)})
        self.assertEqual(chave.health_status(margin_seconds=900), SAUDE_EXPIRANDO)

    def test_a_key_past_its_validity_is_expired(self):
        chave = VirtualKey({"key_alias": "k", "expires": daqui(days=-1)})
        self.assertEqual(chave.health_status(), SAUDE_EXPIRADA)

    def test_a_blocked_key_is_blocked_before_anything_else(self):
        chave = VirtualKey({"key_alias": "k", "blocked": True, "expires": daqui(days=30)})
        self.assertEqual(chave.health_status(), SAUDE_BLOQUEADA)

    def test_a_key_at_its_budget_is_over_budget(self):
        chave = VirtualKey({"key_alias": "k", "spend": 10.0, "max_budget": 10.0})
        self.assertEqual(chave.health_status(), SAUDE_ESTOURADA)

    def test_an_expired_key_that_also_burned_its_budget_reads_as_expired(self):
        chave = VirtualKey({"key_alias": "k", "spend": 99, "max_budget": 10,
                            "expires": daqui(days=-1)})
        self.assertEqual(chave.health_status(), SAUDE_EXPIRADA)

    def test_a_key_with_no_declared_expiry_is_active_and_never_unlimited(self):
        chave = VirtualKey({"key_alias": "k"})
        self.assertEqual(chave.health_status(), SAUDE_ATIVA)
    ...
    def test_a_zero_budget_is_no_budget_not_an_exhausted_one(self):
        chave = VirtualKey({"key_alias": "k", "spend": 0.0, "max_budget": 0})
...
class TestIdentificacaoDaChave(unittest.TestCase):
    def test_the_alias_wins_when_there_is_one(self):
        self.assertEqual(VirtualKey({"key_alias": "producao"}).alias, "producao")

    def test_without_an_alias_the_token_is_masked(self):
        chave = VirtualKey({"token": "sk-SEGREDO-QUE-NAO-PODE-VAZAR-123456"})
...
class TestResumo(unittest.TestCase):
    def test_the_summary_counts_each_state(self):
        chaves = [
            VirtualKey({"key_alias": "a", "expires": daqui(days=30)}),
            VirtualKey({"key_alias": "b", "expires": daqui(minutes=5)}),
            VirtualKey({"key_alias": "c", "expires": daqui(days=-1)}),
            VirtualKey({"key_alias": "d", "blocked": True}),
        ]
        resumo = summarize(chaves, margin_seconds=900)
        self.assertEqual(resumo[SAUDE_ATIVA], 1)
        self.assertEqual(resumo[SAUDE_EXPIRANDO], 1)
        self.assertEqual(resumo[SAUDE_EXPIRADA], 1)
        self.assertEqual(resumo[SAUDE_BLOQUEADA], 1)
