...
```
        ...
        └──────────┬───────────┘
                   ...
                   ▼
   ┌───────────────────────────────┐
   │ client.py   — talks to the API │
   │ models.py   — shapes the rows  │
   │ limits.py   — the coherence rule│
   │ credential_check.py — live probe│
   │ engine.py   — runs the cycle   │
   └───────────────┬───────────────┘
                   │ findings
                   ▼
   ┌───────────────────────────────┐
...
| :--- | :--- |
| `client.py` | The admin API. GET only; tolerates 404 and 500 where those are normal states |
| `models.py` | Turns API rows into objects with the derived properties the screen needs |
| `limits.py` | The coherence rule, field by field, with the message that explains the consequence |
| `credential_check.py` | Live probe of a provider key |
| `engine.py` | Orchestrates the cycle and assembles the findings |
...
| `auth.py` | Basic Auth, password hashing, break-glass recovery |
...
`auth.py`, `logs.py`, `prefs.py`, `i18n.py`, the panel posture, the packaging and the CI are
shared with the siblings — the same problems have the same answers. `client.py` and `limits.py`
are what this project adds.
